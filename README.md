...
It is recommended to just clone the module in the folder. However, if you want to install it as a package instead,
you can install this module by typing:
    
    pip install git+https://github.com/Marganotvke/Simple-Lazy-GSpread-Wrapper#egg=slgsw
If you have trouble installing this module, or just simply do not want to install it, you can also just clone the dist and install it yourself, or clone the module in the 'slgsw' directory and put the module into your project's directory.
...
* slgsw(cred,scp)
    ...
    <br> scp: you will need to fill in the list of 'powers' the service accounts have access to, in the form of a list. eg:
    ...
    <br>Defaults to read only access.
...
* sheet.open_sheet(sp,ws)
    ...
    <br><br>Defaults to the first worksheet. Currently it needs both the sheet and worksheet name to work, even if there's only one worksheet.
...
*   sheet.find_cell(query)
    <br>To find the location of one or more cells that contain the value that you want to find. It returns a list in cell referencing format (A1 notation). eg:
...
The following is an example that I put up. You can also find the example file in the repository:
    ...
    from slgsw import slgsw #import the module
...
<br> Was kinda frustrated to use the gspread api, therefore wrote this wrapper to make it just a tiny bit user-friendly. Does not mean it's a bad api though, it's just a bit too complex for my little brain.
